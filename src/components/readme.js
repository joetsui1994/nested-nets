        ...
        <div className={classes.root}>
            ...
            <Dialog onClose={_closeDialog} open={dialogOpen}>
                ...
                <DialogContent className={classes.dialogContent}>
                    ...
                    the <span style={{ fontFamily: 'Courier' }}>watts_strogatz_graph</span> method in NetworkX. Each link (connecting a pair of nodes, here A and B) is assigned a weight
                    ...
                    large only when both <Latex>$k_A$</Latex> and <Latex>$k_B$</Latex> are large and increases monotonically with both <Latex>$k_A$</Latex> and <Latex>$k_B$</Latex>. This is to simulate
                    ...
                    when both individuals are strongly connected to others and less so when at least one individual is isolated (not sure if this is actually realistic). <span style={{ fontStyle: 'italic', fontWeight: 500 }}>To
                    differentiate between a node in the social network and a node in the environmental network, nodes in the social network will be referred to as particles from hereon.</span><br /><br />
                    ...
                    at their corresponding environmental node until the next "jump" (at regular intervals of <Latex>$\Delta t$</Latex>). At each jump,
                    each particle could either (1) remain at the same node, or (2) move to another node, anywhere in the environmental network. Suppose that a particle A is at
                    ...
                    the local-attraction which determines the importance of social connections to particles that are located at the same node. The more connected particles there are located at the same node,
                    ...
                    where <Latex>{`$P_{A,j}$`}</Latex> is the set of particles that are connected to particle A in the social network and are located at node <Latex>$j$</Latex> at time <Latex>$t$</Latex>, and <Latex>$d(i,j)$</Latex> is
                    ...
                    the tendency of a particle to explore a new node regardless of whether any connected particles are located there. The probability of (2) is therefore proportional to the sum of <Latex>$p(i \rightarrow j)$</Latex> for
                    all <Latex>$j \neq i$</Latex>.
                </DialogContent>
